...
const HEADERS = { Authorization: `Bearer ${ process.env.SENTRY_KEY}` };
const BASE_URL = process.env.SENTRY_RELEASE_BASE_URL;
...
function uploadFile(endpoint, releaseVersion, filePath) {
  ...
  const IGNORE_PATH = staticBase ? `${url.parse(staticBase).path }/` : '';
  const CONFLICT_CODE = 409;

  const fileData = path.parse(filePath);
  const fileName = fileData.name + fileData.ext;
  const sentryFilePath = IGNORE_DOMAIN + IGNORE_PATH + fileName;
...
function uploadToSentry(projectSlug, orgSlug, releaseVersion, assets) {
  const releaseEndpoint = makeUrl(projectSlug, orgSlug);
  const uploadEndpoint = `${releaseEndpoint + releaseVersion }/files/`;
